...
class Laporan extends CI_Controller {
    ...
    // Set orientasi kertas jadi LANDSCAPE
        ...
        $write->save('php://output');
                ...
                $tgl_transaksi = $this->input->post('tgl_transaksi');
                $lapor = $this->MdlTransaksi->getAllTransaksiHari($tgl_transaksi)->result();
                $data['tmpTransaksiHari'] = $lapor;

                $this->load->view('Laporan/laporanPage', $data);
            }
        }
    }
    ...
    public function bulanan() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $action = $this->input->post('action'); // Mengambil nilai dari tombol yang ditekan
    
            if ($action === 'cetak') {
                // Lakukan tindakan cetak
                // Misalnya, redirect ke halaman cetak atau melakukan operasi cetak
